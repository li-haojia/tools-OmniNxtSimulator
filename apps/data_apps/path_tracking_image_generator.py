...
config = {
    ...
    "rt_subframes": 8,
    "image_output_path": "path_tracking_sdg",
    "writer": "BasicWriter",
    "writer_config": {
        "output_dir": "path_tracking_sdg", # This param will be overridden by the config["image_output_path"]
        "camera_params": True,
        "rgb": True,
...
config["writer_config"]["output_dir"] = os.path.abspath(os.path.join(config["data_path"], config["image_output_path"]))
config["trajectory_path"] = os.path.join(config["data_path"], config["trajectory_name"])
...
env_url = os.path.join(config["data_path"], config["env_name"])
if stage_utils.open_stage(env_url):
    stage = omni.usd.get_context().get_stage()
else:
    stage = Usd.Stage.Open(env_url)
...
for camera_config in config["robot"]["cameras"]:
    ...
    if projection_type == "fisheye_polynomial":
        ...
        camera_kwargs["clipping_range"] = camera_config.get("clipping_range", (0.001, 1000.0))
